...
SCHEMA_VERSION = 32
# Public inference package version. The numeric schema and accepted legacy
# checkpoint contract keep existing trained weights binary-compatible.
TRAINER_VERSION = "1.0"
MODEL_KIND = "ibms_structured_v1_0"
COMPATIBLE_CHECKPOINT_MODEL_KINDS = frozenset((
    MODEL_KIND,
    "ibms_structured_v3_2",  # Existing trained checkpoints use this contract id.
))
...
def require_v1(value: dict, name: str = "数据") -> None:
    if (value.get("schema_version") != SCHEMA_VERSION
            or value.get("model_kind") not in COMPATIBLE_CHECKPOINT_MODEL_KINDS):
        raise ValueError(
            f"{name} 不是 MuseChartWeaver v1.0 可用模型 "
            f"(schema_version=32, model_kind={MODEL_KIND})"
        )
    vocabulary = value.get("vocabulary", value)
    if vocabulary.get("semantic_types") != list(SEMANTIC_TYPES):
        raise ValueError(f"{name} 的 semantic_types 与 v1.0 模型不一致")
    if (vocabulary.get("ibms_ids") != list(IBMS_IDS)
            or vocabulary.get("boss_control_ids") != list(BOSS_CONTROL_IDS)):
        raise ValueError(f"{name} 的 IBMS/Boss 白名单或顺序与 v1.0 不一致")
    ...
    if vocabulary.get("event_bundles") != [list(value) for value in EVENT_BUNDLES]:
        raise ValueError(f"{name} 的 v1.0 联合事件词表不匹配")
...
# Compatibility aliases for older callers; public naming is v1.0.
require_v3 = require_v1
require_v32 = require_v1
